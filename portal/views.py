...
def schedule(request,slug=""):
    ...
    if request.method == 'POST':
        ...
        if(len(i1) != 0):
            msg = "Interview cannot be scheduled beacuse "+interviewer.name+ ' has an interview scheduled on '+ schedule_date+ ' at '+ startTimer
            messages.error(request, msg)
        if(len(i2) != 0):
            msg = "Interview cannot be scheduled beacuse "+candidate.name+ ' has an interview scheduled on '+ schedule_date+ " at "+ startTimer
            messages.error(request, msg)
    return redirect('home')
